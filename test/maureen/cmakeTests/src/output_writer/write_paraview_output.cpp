...
void writeParaviewOutput(int fileNo)
{
    ...
    // Set the number of pressure values and allocate memory for it. 
    // We already know the number, it has to be the same as there are nodes in the mesh.
    arrayPressure->SetNumberOfTuples(dataSet->GetNumberOfPoints());

    arrayPressure->SetName("pressure");

    // loop over the nodes of the mesh and assign an artificial value that changes with fileNo
    for (int j = 0; j < 10; j++)
    {
        for (int i = 0; i < 10; i++)
        {
