...
void RedBlackSOR::solve()
{
  ...
  do
  {
    ...
    for (int j = discretization_->pJBegin() + 1; j < discretization_->pJEnd(); j++)
    {
      ...
      for (int i = discretization_->pIBegin() + ((j - sor_offset) % 2) + 1 ; i < discretization_->pIEnd(); i += 2)
      {
    ...
    for (int j = discretization_->pJBegin() + 1; j < discretization_->pJEnd(); j++)
    {
      ...
      for (int i = discretization_->pIBegin() + ((j - 1 + sor_offset) % 2) + 1; i < discretization_->pIEnd(); i += 2)
  ...
  } while (res2_global >= epsilon2 && iteration < maximumNumberOfIterations_);
  //} while (res2_local >= epsilon2 && iteration < maximumNumberOfIterations_);
}